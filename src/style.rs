    ...
    fn expand(
        ...
        let left =
            (width as usize).saturating_sub(match cur.lines().find(|line| line.contains('\x00')) {
                Some(line) => Width::new().ansi_str(&line.replace('\x00', "")),
                None => Width::new().ansi_str(&cur),
            });
        match self {
...
impl fmt::Display for PaddedStringDisplay<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let cols = Width::new().ansi_str(self.str);
...
pub fn write_ansi_range(
    ...
    for (s, is_ansi) in AnsiCodeIterator::new(text) {
        ...
        let width_calculator = Width::new();
        for c in s.chars() {
            let c_width = width_calculator.char(c);
...
pub static WIDTH: Width = Width::new();
...
impl Width {
    ...
    pub fn set_cjk(&self, is_cjk: bool) {
        self.is_cjk.store(is_cjk, Ordering::Relaxed);
    }

    /// Whether the alternate width calculation more suited to CJK contexts is enabled
    pub fn is_cjk(&self) -> bool {
        self.is_cjk.load(Ordering::Relaxed)
    }
}
